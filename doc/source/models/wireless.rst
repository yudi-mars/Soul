...
介绍：
    适用于边缘场景的脉冲多层感知机网络结构，针对无线信号数据任务进行了定制优化。
特殊配置：
...
介绍：
    适用于边缘场景的脉冲LeNet网络结构，针对无线信号数据任务进行了定制优化。
特殊配置：
...
介绍：
    适用于边缘场景的脉冲循环神经网络结构，针对无线信号数据任务进行了定制优化。
特殊配置：
...
介绍：
    适用于边缘场景的脉冲卷积循环神经网络结构，针对无线信号数据任务进行了定制优化。
特殊配置：
    * hidden_dim:128 （int）内部循环层的隐藏维度
    * num_layers:1 （int）循环层数量
...
介绍：
    适用于无线信号数据分类任务的 Transformer 架构脉冲神经网络模型实现。
