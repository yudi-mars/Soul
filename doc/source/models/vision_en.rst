...
The following are the visual image processing models supported by the Soul toolkit:
...
Description：
    A spike-based multi-layer perceptron network architecture customized and optimized for visual image processing tasks in edge scenarios.
...
Description：
    A spike-based LeNet network architecture customized and optimized for visual image processing tasks in edge scenarios.
Special Configurations：
...
Description：
    A spike-based recurrent neural network architecture customized and optimized for visual image processing tasks in edge scenarios.
Special Configurations：
...
Description：
    A spike-based convolutional recurrent neural network architecture customized and optimized for visual image processing tasks in edge scenarios.
Special Configurations：
    * hidden_dim:128 （int）hidden dimension for interior recurrent layers
    * num_layers:1 （int）the number of recurrent layers
...
Usage:
    Set the --model or -m parameter to: spikingvgg5, spikingvgg9, spikingvgg11, spikingvgg13, spikingvgg16, spikingvgg19
Number of Parameters：
...
Usage:
    Set the --model or -m parameter to: sewresnet18, sewresnet34, sewresnet50
Number of Parameters：
...
Usage:
    Set the --model or -m parameter to: msresnet18, msresnet34, msresnet50
Number of Parameters：
...
Description：
    Implementation of a Transformer-structured spiking neural network model for image data classification tasks.
    ...
    computation logic in neuromorphic chips.Hence, we use a normal linear layer to replace the RepConv part to 
    achieve stable model implementation.
Special Configurations：
    * mlp_ratio:4 （float）expand ratio for hidden dimension in MLP intermediate layers
    * num_heads:8 （int）number of heads for attention output
