...
介绍：
    适用于边缘场景的脉冲多层感知机网络结构，针对动作行为任务进行了定制优化。
特殊配置：
...
介绍：
    适用于边缘场景的脉冲LeNet网络结构，针对动作行为任务进行了定制优化。
特殊配置：
...
介绍：
    适用于边缘场景的脉冲循环神经网络结构，针对动作行为任务进行了定制优化。
特殊配置：
...
介绍：
    适用于边缘场景的脉冲卷积循环神经网络结构，针对动作行为任务进行了定制优化。
特殊配置：
    * hidden_dim:128 （int）内部循环层的隐藏维度
    * num_layers:1 （int）循环层数量
...
介绍：
    适用于人体活动识别任务的脉冲级深度卷积神经网络架构模型实现。
特殊配置：
    * mlp_hidden_dim:400 （int）多层感知机部分中间层的隐藏维度
...
使用方法：
    设置--model或-m：sensehar
参数量：

介绍：
    适用于人体活动识别任务的脉冲级 SenseHAR 模型实现。
特殊配置：
    * mlp_hidden_dim:400 （int）多层感知机部分中间层的隐藏维度
    * k_pool:2 （int）K最大池化操作的卷积核尺寸
    * encoder_channels:64 （int）编码器滤波器的中间输出通道数
...
介绍：
    适用于人体活动识别任务的脉冲级深度卷积-长短期记忆网络实现
特殊配置：
    * hidden_dim:128 （int）长短期记忆网络隐藏层的维度
    * n_channels:64 （int）卷积层的中间通道数
...
使用方法：
    设置--model或-m：sewresnet34、sewresnet50
参数量：

介绍：
    适用于人体活动识别任务的ResNet架构网络实现。
特殊配置：
...
使用方法：
    设置--model或-m：msresnet34、msresnet50
参数量：
...
使用方法：
    设置--model或-m：ispikformer、spikformer256、spikformer384、spikformer512
参数量：

介绍：
    适用于人体活动识别任务的脉冲级Transformer模型实现。
特殊配置：
    * embedding_dim:256 （int）嵌入维度
    * num_head:8 （int）注意力头数
    * depth:4 （int）Transformer块数量
    * mlp_ratio:4.0 （float）MLP中间层隐藏维度的扩展系数
...
使用方法：
    设置--model或-m：metaspikeformer256、metaspikeformer384
参数量：

介绍：
    适用于动作数据分类任务的 Transformer 架构脉冲神经网络模型实现。
...
使用方法：
    设置--model或-m：spikingresformer256、spikingresformer384
参数量：
...
使用方法：
    设置--model或-m：qkformer256、qkformer384
参数量：
