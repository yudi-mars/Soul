...
__all__ = ['QKFormer', 'QKFormer256', 'QKFormer384']

class PatchEmbedInit(nn.Module):
    def __init__(self, lif, img_size=128, patch_size=4, in_channels=2, embed_dims=256):
        super().__init__()
        ...
        self.C = in_channels
        self.N = self.image_size // patch_size
        self.num_patches = self.N
        ...
        # self.proj_maxpool = nn.MaxPool1d(kernel_size=3, stride=2, padding=1, dilation=1, ceil_mode=False)
        self.proj_lif = deepcopy(lif)

        self.proj1_conv = nn.Conv1d(embed_dims // 2, embed_dims, kernel_size=3, stride=1, padding=1, bias=False)
        self.proj1_bn = nn.BatchNorm1d(embed_dims)
        # self.proj1_maxpool = nn.MaxPool1d(kernel_size=3, stride=2, padding=1, dilation=1, ceil_mode=False)
        self.proj1_lif = deepcopy(lif)

        self.proj2_conv = nn.Conv1d(embed_dims, embed_dims, kernel_size=3, stride=1, padding=1, bias=False)
        self.proj2_bn = nn.BatchNorm1d(embed_dims)
        self.proj2_lif = deepcopy(lif)

        self.proj_res_conv = nn.Conv1d(embed_dims // 2, embed_dims, kernel_size=3, stride=1, padding=1, bias=False)
    ...
    def forward(self, x):
        ...
        x = self.proj2_conv(x)
        x = self.proj2_bn(x).reshape(T, B, -1, N).contiguous()
        x = self.proj2_lif(x)
...
class PatchEmbeddingStage(nn.Module):
    def __init__(self, lif, img_size=128, patch_size=4, in_channels=2, embed_dims=256):
        ...
        # self.proj3_maxpool = nn.MaxPool1d(kernel_size=3, stride=2, padding=1, dilation=1, ceil_mode=False)
    ...
    def forward(self, x):
        ...
        x = self.proj3_conv(x)
        x = self.proj3_bn(x)
        # x = self.proj3_maxpool(x).reshape(T, B, -1, N // 2).contiguous()
        x = x.reshape(T, B, -1, N).contiguous()
        x = self.proj3_lif(x).flatten(0, 1).contiguous()

        x = self.proj4_conv(x)
        x = self.proj4_bn(x).reshape(T, B, -1, N).contiguous()
        x = self.proj4_lif(x)
...
class QKFormer(nn.Module):
    def __init__(self, config, depths=4, embed_dims=384, norm_layer=partial(nn.LayerNorm, eps=1e-6)):
        ...
        img_size = config['input_channels']
        lif = config['neuron']
