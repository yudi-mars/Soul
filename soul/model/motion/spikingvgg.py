...
class ConvBNLIF(nn.Module):
    def __init__(self, lif, in_channels, out_channels):
        ...
        self.conv = nn.Sequential(
            ...
            nn.BatchNorm1d(out_channels, eps=1e-5, momentum=0.1, affine=True, track_running_stats=True)
        )
...
class ConvMLP(nn.Module):
    def __init__(self, lif, in_features=512, out_features=4096, kernel_size=7, mlp_ratio=1.0):
        ...
        self.fc1 = nn.Sequential(
            nn.Conv1d(in_features, mid_features, kernel_size, bias=False),
            nn.BatchNorm1d(mid_features),
        )
        self.sn1 = deepcopy(lif)

        self.fc2 = nn.Sequential(
            nn.Conv1d(mid_features, out_features, 1, bias=False),
    ...
    def forward(self, x):
        # default multi-step mode, the input shape of x from Conv layers must be (T, B, C, N)
        T, B, C, N = x.shape
    ...
    def __init__(self, cfg, config):
        ...
        for v in cfg:
            ...
            if v == 'M':
                ...
                v = int(v)
                layers += [ConvBNLIF(config['neuron'], prev_chs, v * input_dim)]
                prev_chs = v * input_dim
