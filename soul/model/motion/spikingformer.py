...
Description:
    Implementation of Spikingformer (spike-driven Transformer SNN) for motion domain.

    Input adaptation:
    - Motion input: (T, B, C, D) where C=sensor_channels, D=temporal_length
    - Unsqueezed to: (T, B, C, D, 1) for Conv2d tokenizer
    - Uses input_channels as H, input_dim as W for the tokenizer
...
class Spikingformer(nn.Module):
    """
    Spikingformer for motion domain.

    Motion input: (T, B, C, D) where C=sensor_channels, D=temporal_length.
    Unsqueezed to (T, B, C, D, 1) for 2D convolutional tokenizer.
    """
    def __init__(self, config, depths=4, embed_dims=384, drop_path_rate=0., norm_layer=nn.LayerNorm):
        super().__init__()
        num_classes = config['num_classes']
        self.T = config['time_step']
        # Motion: input_channels=C (sensors), input_dim=D (temporal length)
        # Treat as H=input_channels, W=input_dim, C=1
        img_size_h = config['input_channels']
        img_size_w = config['input_dim']
        in_channels = 1
        lif = config['neuron']
    ...
    def forward(self, x):
        # Motion input: (T, B, C, D) -> (T, B, C, D, 1)
        x = x.unsqueeze(-1)
