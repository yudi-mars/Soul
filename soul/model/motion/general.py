...
class SpikingMLP(nn.Module):
    def __init__(self, config):
        ...
        C, D = config['input_channels'], config['input_dim']
        lif = config['neuron']

        self.ln1 = nn.Linear(C * D, config['hidden_dim'])
...
class SpikingLeNet(nn.Module):
    def __init__(self, config):
        ...
        self.conv1 = nn.Conv2d(1, 32, kernel_size=(1, 5), stride=1, padding=0)
        ...
        self.conv3 = nn.Conv2d(64, 96, kernel_size=(1, 5), stride=1, padding=0)
...
class SpikingRNN(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.permute(0, 1, 3, 2) # (T, B, C, L) -> (T, B, L, C)

        x = self.rnn(x) # -> (T, B, L, D)
...
class SpikingConvRNN(nn.Module):
    def __init__(self, config):
        ...
        self.conv1 = nn.Conv2d(1, 32, kernel_size=(1, 3), stride=1, padding=(0, 1))
        self.bn1 = nn.BatchNorm2d(32)
        self.lif1 = deepcopy(lif)
        ...
        self.conv3 = nn.Conv2d(64, 96, kernel_size=(1, 3), stride=1, padding=(0, 1))
        self.bn3 = nn.BatchNorm2d(96)
        self.lif3 = deepcopy(lif)
    ...
    def forward(self, x):
        ...
        x = x.permute(0, 1, 4, 2, 3).contiguous() # -> (T, B, W, C, H)
