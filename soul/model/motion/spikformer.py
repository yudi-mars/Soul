...
class SPS(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.proj_conv1(x)
        w1 = x.shape[-1]
        x = self.proj_bn1(x).reshape(T, B, -1, w1).contiguous()
        x = self.proj_lif1(x).flatten(0, 1).contiguous()

        x = self.proj_conv2(x)
        w2 = x.shape[-1]
        x = self.proj_bn2(x).reshape(T, B, -1, w2).contiguous()
        x = self.proj_lif2(x).flatten(0, 1).contiguous()
        x = self.maxpool2(x)

        x = self.proj_conv3(x)
        w3 = x.shape[-1]
        x = self.proj_bn3(x).reshape(T, B, -1, w3).contiguous()
        x = self.proj_lif3(x).flatten(0, 1).contiguous()
        x = self.maxpool3(x)
        # TB, D, N
        n = x.shape[-1]
        x_feat = x.reshape(T, B, -1, n).contiguous()
        x = self.rpe_conv(x)
        x = self.rpe_bn(x).reshape(T, B, -1, n).contiguous()
        x = self.rpe_lif(x)
        x = x + x_feat
        # (T, B, D, N) -> (T, B, N, D)
