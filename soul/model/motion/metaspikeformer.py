...
__all__ = ['MetaSpikeformer', 'MetaSpikeformer256', 'MetaSpikeformer384']
...
class DownSampling(nn.Module):
    def __init__(self, lif, in_channels, embed_dim, kernel_size=3, stride=2, padding=1, first_layer=True):
        ...
        self.encode_bn = nn.BatchNorm1d(embed_dim)
        if not first_layer:
...
class SepConv(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.lif1(x)
        x = x.flatten(0, 1)
        x = self.bn1(self.pwconv1(x)).reshape(T, B, -1, N)
        x = self.lif2(x)
        x = x.flatten(0, 1)
        x = self.dwconv(x)
...
class ConvBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.Conv(x) + x
        x_feat = x
        x = self.bn1(self.conv1(self.lif1(x).flatten(0, 1))).reshape(T, B, self.mlp_ratio * C, N)
...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.transpose(2, 3).reshape(T, B, C, N).contiguous()
