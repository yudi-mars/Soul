...
class DSSA(nn.Module):
    ...
    def forward(self, x):
        ...
        # L = number of patch tokens (after W conv), i.e., attn.shape[-2]
        L = attn.shape[-2]
        scale2 = 1. / torch.sqrt(self.firing_rate_attn * L)
...
class SpikingResformer(nn.Module):
    ...
    def forward_features(self, x):
        ...
        x = multi_time_forward(x, self.prologue)  # [T,B,C,H,W]

        # stage-by-stage forward, pad AFTER downsample and BEFORE DSSA
        for idx, stage in enumerate(self.layers):
            # stage is nn.Sequential
            modules = list(stage.children())

            if idx != 0:
                # first module is DownsampleLayer
                x = modules[0](x)
                modules = modules[1:]

            # pad to patch multiple for this stage (needed by DSSA's W conv stride=patch)
            patch = self.patch_sizes[idx]
            x = self._pad_tbchw_to_multiple(x, patch)

            # run the rest blocks (DSSA/GWFFN interleaving)
            for m in modules:
                x = m(x)
    ...
    def forward(self, x):
        x = self.forward_features(x) # [T, B, C, 1, 1]  
        x = self.forward_head(x)

        return x

    def _pad_tbchw_to_multiple(self, x: torch.Tensor, multiple: int) -> torch.Tensor:
        # x: [T,B,C,H,W]
        T, B, C, H, W = x.shape
        ...
        y = x.flatten(0, 1)  # [T*B, C, H, W]
        y = F.pad(y, (0, pad_w, 0, pad_h))  # pad right, then bottom
        return y.view(T, B, C, H + pad_h, W + pad_w)
