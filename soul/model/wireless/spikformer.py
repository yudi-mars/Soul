...
class SSA(nn.Module):
    def __init__(self, lif, dim, num_heads=8):
        ...
        if hasattr(self.attn_lif, "v_threshold") and isinstance(self.attn_lif.v_threshold, nn.Parameter):
            with torch.no_grad():
                self.attn_lif.v_threshold.fill_(0.5)
...
class SPS(nn.Module):
    ...
    def forward(self, x):
        ...
        x_feat = x.reshape(T, B, -1, h4, w4).contiguous()

        x = self.rpe_conv(x)
        x = self.rpe_bn(x).reshape(T, B, -1, h4, w4).contiguous()
        x = self.rpe_lif(x)
        x = x + x_feat
