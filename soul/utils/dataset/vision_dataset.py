...
@register_dataset('ncaltech')
class iNCaltech101(VisionData):
    def __init__(self, data_dir, coding_schema, time_step, receptor_size=None):
        super().__init__(data_dir, coding_schema, time_step, receptor_size)
    ...
    def download_data(self):
        ...
        NCaltech101(self.data_dir, data_type='frame', frames_number=self.T, split_by='number')
...
@register_dataset('nmnist')
class iNMNIST(VisionData):
    def __init__(self, data_dir, coding_schema, time_step, receptor_size=None):
        super().__init__(data_dir, coding_schema, time_step, receptor_size)
        ...
        self.train_trsf = [
            transforms.Resize(
                size=(self.receptor_size if self.receptor_size else 34, self.receptor_size if self.receptor_size else 34), 
                interpolation=transforms.InterpolationMode.NEAREST),
            DVSAugment(degree=15, radius=3), 
        ]
        self.test_trsf = [
        ...
        self.input_shape = (
            2, 
            self.receptor_size if self.receptor_size else 34, 
            self.receptor_size if self.receptor_size else 34)
        self.num_classes = 10

    def download_data(self):
        NMNIST(self.data_dir, data_type='frame', train=True, frames_number=self.T, split_by='number') 
