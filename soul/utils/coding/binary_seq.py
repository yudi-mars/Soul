'''
Filename: binary_seq.py
Author: Weisong Zhang <zws1415@zju.edu.cn>
Date Created: 2025-11-19
Description:
    Sequential binary code: quantize each value to B-bit integer and emit bits
    across time steps (t indexes bit positions). Input in [0,1].
...
@torch.no_grad()
def encode(inputs: torch.Tensor, num_steps=False, bits=None) -> torch.Tensor:
    T = int(num_steps) if num_steps and num_steps>0 else 8
    B = bits if bits else min(8, T)
    ...
    if x.dim()==3:
        ...
        raise ValueError(f"binary_seq expects (C,H,W) or (W,C).")
    pc = torch.clamp(torch.nan_to_num(pc),0,1)
    code = torch.round(pc * (2**B - 1)).to(torch.int64)  # [P,C]
    spikes = torch.zeros((T, pc.shape[0], pc.shape[1]), device=pc.device, dtype=pc.dtype)
    for t in range(B):
        mask = ((code >> t) & 1).to(pc.dtype)
        spikes[t] = mask
