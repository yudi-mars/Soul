...
for neuron_type in "LIF" "PLIF" "GatedLIF" "CLIF"; do
    for seed in 41 42 43; do
        GPU_ID=${GPUS[$((TASK_COUNT % NUM_GPUS))]}  # 轮流分配 GPU
        echo "Running training with neuron_type=$neuron_type on GPU $GPU_ID"

        CUDA_VISIBLE_DEVICES=$GPU_ID python train.py --T 8 \
                                                     --num_classes 101 \
                                                     --epochs 100 \
                                                     --batch_size 32 \
                                                     --dataset "NCaltech101" \
                                                     --data_dir "./data/NCaltech101" \
                                                     --lr 5e-4 \
                                                     --neuron_type "$neuron_type" \
                                                     --seed "$seed" \
                                                     --model_type "SpikingVGG9" > "ncaltech101_logs/ncaltech101_${neuron_type}_${seed}.log" & 
